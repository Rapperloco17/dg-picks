...
// Función para obtener equipos con mejor forma
async function getTopFormTeams() {
  try {
    const teams = await prisma.team.findMany({
      where: {
        form: {
...
export default async function TeamsPage() {
  ...
  const topFormTeams = await getTopFormTeams();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-500">
            {teams.length > 0 ? `${teams.length} teams in database` : 'Browse team profiles and statistics'}
          </p>
        </div>
      </div>
        ...
        <Card className="glass border-0">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-zinc-500 mb-4">
              Sync standings first to populate teams data.
            </p>
            ...
            <Card className="glass border-0">
              ...
              <CardContent>
                <div className="space-y-3">
                  {topFormTeams.slice(0, 5).map((team, i) => (
